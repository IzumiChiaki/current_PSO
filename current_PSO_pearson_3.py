# -*- coding: utf-8 -*-
"""
Created on Fri Apr  5 15:09:36 2019
    ...
    def Func(self,factor):
        ...
        for i in range(samplenum):
            f11 = (-factor[0] * self.data[i][0]+factor[1] * self.data[i][1] - \
                   factor[2] * self.data[i][2]) * self.data[i][3]
                   
            f1.append(f11)
            f22=  (factor[0] * self.data[i][0]+factor[1] * self.data[i][1] + \
                              factor[2] * self.data[i][2]) *(factor[0] * self.data[i][0] + \
                              factor[1] * self.data[i][1] + factor[2] * self.data[i][2])
            f2.append(f22)
            f33=   self.data[i][3]*self.data[i][3]
            f3.append(f33)                  
        fenzi = sum(f1)
...
if __name__ == "__main__":
    starttime = time.time()
    data = file2matrix('THAQIPM25I3.txt')
    result = []
    for i in range(10):
        print("iter",i)
        a = MyPSO(60,3,0.9,0.4,2.05,2.05,100,data=data)
        result.append(a.Run())
    endtime = time.time()
    res = max(result)
    dataframe = pd.DataFrame({'Pearson':[res[0]],'T factor':[res[1][0]],\
                              'H factor':[res[1][1]],'PM2.5 factor':[res[1][2]]})
    dataframe.to_csv("result_PSO_pearson_3.csv",mode='a',index=False,header=False,sep=',',\
                     columns=['T factor','H factor','PM2.5 factor','Pearson'])
    print(res)
