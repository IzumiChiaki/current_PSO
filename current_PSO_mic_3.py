...
class MyPSO(object):
    ...
    def Func(self,factor):
        E_list = self.data.values[:,0:3].astype(np.float64).dot(np.array(factor).T)
        pr_list = []
...
if __name__ == "__main__":
    ...
    for i in range(200):
        print("iter",i)
        a = MyPSO(60,3,0.9,0.4,2.05,2.05,200,data=data)
        result.append(a.Run())
    res = max(result)
    endtime = time.time()
    dataframe = pd.DataFrame({'MIC':[res[0]],'T factor':[res[1][0]],\
                              'H factor':[res[1][1]],'PM2.5 factor':[res[1][2]]})
    dataframe.to_csv("result_PSO_mic_3.csv",mode='a',index=False,header=False,sep=',',\
