...
class MyPSO(object):
    ...
    def Func(self,factor):
        ...
        E_list = np.sqrt(data_p[:,0:4].astype(np.float64).dot(np.array(factor).T))
        pr_list = []
...
if __name__ == "__main__":
    starttime = time.time()
    data = pd.read_csv('current_PSO_mic_4_test.csv')
    result = []
    for i in range(10):
        print("iter",i)
        a = MyPSO(60,4,0.9,0.4,2.05,2.05,500,data=data)
        result.append(a.Run())
    res = max(result)
    endtime = time.time()
    dataframe = pd.DataFrame({'MIC':[res[0]],'T factor':[res[1][0]],\
                              'H factor':[res[1][1]],'O3 factor':[res[1][2]],\
                              'PM2.5 factor':[res[1][3]]})
    dataframe.to_csv("result_PSO_mic_4_test.csv",index=False,sep=',',\
                     columns=['T factor','H factor','O3 factor','PM2.5 factor','MIC'])
    print(res)
